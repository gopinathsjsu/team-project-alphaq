...
router.post('/viewPrice', async (req, res) => {
  try {
    const { userId, showId, seats, child, adult, senior, usingRewardPoints } =
      req.body;
...
router.post('/confirm', validateBooking, async (req, res) => {
  try {
    ...
    const totalTicketCost = pricingResponse.checkout.find(
      (item) => item.title === 'Tickets cost',
    ).cost;
