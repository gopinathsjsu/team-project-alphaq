...
export default function MovieNavbar() {
  ...
  return (
    <nav
      className="top-0 absolute z-50 w-full flex flex-wrap items-center justify-between px-6 py-3
    navbar-expand-lg  border"
    >
      <div className="container px-4 mx-auto flex flex-wrap items-center justify-between ">
        <div className="w-full  relative flex justify-between lg:w-auto lg:static lg:block lg:justify-start">
          <Link
            className="text-white text-sm font-bold leading-relaxed inline-block mr-4 ml-3
              whitespace-no-wrap uppercase"
            to="/"
          >
            <div className="flex ">
          ...
          <button
            className="cursor-pointer text-xl leading-none px-3 py-1 border border-solid border-transparen
             rounded  block lg:hidden outline-none focus:outline-none"
            type="button"
            onClick={() => setNavbarOpen(!navbarOpen)}
          >
            <i className="text-gray-600 fas fa-bars" />
            {/* <i
                className={
                  "text-gray-600 fas" + (navbarOpen ? "fa-times" : "fa-bars")
                }
              /> */}
          </button>
        </div>
        ...
        >
          <ul className="flex flex-col lg:flex-row list-none lg:ml-auto">
            ...
            {/* <li className="flex items-center">
              <Link
                to="/createclub"
                className="hover:text-lightbeta text-gray-700 md:text-white-400 px-3 py-4 lg:py-2
                 sm:text-white-400 flex items-center text-xs uppercase font-bold"
                href="#"
              >
            ...
            <li className="flex items-center">
              <Link
                to="/clubsearch"
                className="hover:text-lightbeta text-gray-700 px-3 py-4 lg:py-2 flex items-center
              ...
              <Link to="/auth/login">
                <button
                  className="bg-white hover:bg-offwhite text-gray-800 active:bg-gray-100 text-xs font-bold
                   uppercase px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none lg:mr-1
